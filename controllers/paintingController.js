...
async function generatePaintings(req, res) {
  ...
  try {
    ...
    const generatePaintingsIdeas = async () => {
      ...
      for (let i = 0; i < quantity; i++) {
        ...
        if (paintingParams.some((p) => p === undefined)) {
          console.error(
            "Attempted to execute query with undefined parameter:",
            { paintingParams }
          );
          return res.status(500).json({
            error: "Internal server error: Invalid query parameter detected",
      ...
      const processIdeas = async () => {
        ...
        const startNextIdea = () => {
          ...
          const promise = openAIService
            .generateImage(idea.id, idea.fullPrompt, references)
            .catch((error) =>
              console.error(
                `Error generating image for idea ${idea.id}:`,
                error
              )
            )
            .finally(() => {
    ...
    // Generate Painiting Ideas in the Background
    generatePaintingsIdeas();
